...
const ResizeObserver: FunctionComponent<ResizeObserverProps> = (props: ResizeObserverProps) => {
    const wrapperRef = useRef<HTMLDivElement>()

    useEffect(() => {
        if (wrapperRef.current == undefined) return

        const observer = new SizeObserver((entries): void => props.onChange(entries))
